...
| `gui.py` | Desktop GUI (PySide6) over the same core: connection bar (usb/ble + token), device/status panels, tabs **Functions / Console / Files / Stream / Analyze**. Functions = group list + a button (with arg fields) for every command; danger commands confirm. Files = a **device file browser** (navigate dirs, download/view/delete) + manual get/put. Stream = telemetry/wifi/nrf/rf with live host analysis. All device I/O on one worker thread. Run: `python gui.py --port /dev/ttyACM0`. |
